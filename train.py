...
def fit_model(features, labels, test, epochs):
    ###########
    #Model Building
    ###########
    model = Sequential()
    ...
    model.fit(x=features, 
          y=labels, 
          epochs=epochs,
          validation_data=test, verbose=1 ,callbacks=[early_stop])
...
def main(args):
    ...
    model = fit_model(scaled_X_train, y_train, (scaled_X_test, y_test), epochs)
